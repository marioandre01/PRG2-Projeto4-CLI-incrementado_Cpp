 ...
 * Author: root
 *
 * Created on 12 de Julho de 2016, 10:22
 */
...
template <typename T> T& TabelaHash<T>::obtem(const string& chave) const{
    ...
    //Usando iterador da lista
    unsigned int linha = hash(chave);
    
    tab[linha].inicia();
    
    while (not tab[linha].fim()) {
        Par & dado = tab[linha].proximo();
        if (chave == dado.chave){
            return dado.valor;
        }
    } 
    
    throw -1; // nao encontrou "dado"
...
template <typename T> T TabelaHash<T>::remove(const string& chave) {
  // pode ser resolvido facilmente usando iteração e método ''remove'' da Lista
    //T conteudoChave;
    int n = 0;
    unsigned int linha = hash(chave);
    ...
    while (not tab[linha].fim()) {
        Par & dado = tab[linha].proximo();
        if (chave == dado.chave){
            Par conteudoLista = tab[linha].remove(n);
            
            return conteudoLista.valor;
        }
        n++;
    } 
}
...
template <typename T> Lista<string> * TabelaHash<T>::chaves() const {
  // Retorna um ponteiro para uma lista com todas as chaves existentes na tabela hash
    Lista<string> * lchaves = new Lista<string>;
    
    for(int i=0; i < linhas; i++){
        tab[i].inicia();
        while (not tab[i].fim()) {
            Par & dado = tab[i].proximo();
            lchaves->adiciona(dado.chave);
        } 
    }
...
template <typename T> Lista<T> * TabelaHash<T>::valores() const {
  // Retorna um ponteiro para uma lista com todas os dados existentes na tabela hash
    Lista<T> * lvalores = new Lista<T>;
    
    for(int i=0; i < linhas; i++){
        tab[i].inicia();
        while (not tab[i].fim()) {
            Par & dado = tab[i].proximo();
            lvalores->adiciona(dado.valor);
        } 
    }
...
template <typename T> void TabelaHash<T>::esvazia(){
// esvazia a tabela
    for(int i=0; i < linhas; i++){
       tab[i].esvazia();
    } 
